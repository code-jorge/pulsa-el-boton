...
const ArrowIcon = ({ className }) => (
  <svg className={className} viewBox="0 0 50 50">
    <path d="M35.71,10.71a1,1,0,0,1-1.41,0L28,4.41V17.69A8.94,8.94,0,0,1,25.36,24l-7.31,7.31A7,7,0,0,0,16,36.31V48a1,1,0,0,1-2,0V36.31A8.94,8.94,0,0,1,16.64,30l7.31-7.31A7,7,0,0,0,26,17.69V4.41l-6.29,6.29a1,1,0,0,1-1.41-1.41l8-8a1,1,0,0,1,1.41,0l8,8A1,1,0,0,1,35.71,10.71Z" />
  </svg>
)
...
const Dilemma = () => {
  ...
  const { isLoading, isError, data } = useQuery({
    queryKey: ['dilemma', slug],
    queryFn: () => getDilemma(slug),
  })

  const [buttonState, setButtonState] = useState('opened')

  const submitVote = useMutation({
    mutationFn: choice => addVote({ choice, dilemma: slug }),
  })

  const clickHandler = choice => () => {
    submitVote.mutate(choice, {
      onSuccess: () => navigate(`/estadisticas/${slug}`),
    })
  }

  return (
    <PageContent
      loading={[isLoading, submitVote.isPending]}
