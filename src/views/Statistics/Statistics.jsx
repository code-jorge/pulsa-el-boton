...
const Statistics = () => {
  ...
  const {
    ...
    queryKey: ['statistics', slug],
    queryFn: () => getVotes(slug),
  })
  ...
  const {
    ...
    queryKey: ['dilemma-next', slug],
    queryFn: () => getNextDilemma(slug),
  })
  ...
  const StatisticsShow = ({ type, totals }) => {
    if (type === 'classic') return <StatisticsShowClassic totals={totals} />
    return null
  }

  const handleClick = () => {
    if (nextData.end) navigate('/final')
    else navigate(`/dilema/${nextData.slug}`)
  }
